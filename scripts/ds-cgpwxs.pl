...
# check all inputs exist and have relevant files (bai/csi/crai + bas)
for my $base_af(($opts{'t'}, $opts{'n'})) {
  if($base_af =~ m/\.bam$/) {
    # must be bai or csi index
    die "ERROR: No index (bai/csi) found for $base_af\n" unless(-e "$base_af.bai" || -e "$base_af.csi");
  }
  else {
    # cram only have crai as index
    die "ERROR: No index (crai) found for $base_af\n" unless(-e "$base_af.crai");
  }
